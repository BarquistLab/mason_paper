...
startot <- off_targets %>% filter(locus_tag %in% rownames(y)) %>% filter(trans_coord %in% c(-20:5)) %>% filter(!grepl("_rev", probe_id))
```
...
for (resname in names(res_acpP_mutants)){
  ...
  #pval distributions:
  hist(restab$PValue, breaks=100, main=resname)
  list_volc[[resname]] <- do_volcano(restab, targetgene = c("acpP","fabF"), #title=resname, 
                   x_limit = 5, show_sig = T,# phopq = phopqvolc,
                   y_limit = 25,
                   alpha=0.001, pointsize = 3,
                   off_target_list = OT)
...
svg("../analysis/volcs_salm_mutagens.svg", width = 18, height = 14)
ggarrange(list_volc$acpP_scrambled_vs_ctrl, list_volc$acpP_vs_ctrl, list_volc$acpP_PNA_mut_1_vs_ctrl, list_volc$acpP_PNA_mut_2_vs_ctrl, list_volc$acpP_PNA_mut_3_vs_ctrl, list_volc$acpP_PNA_mut_4_vs_ctrl, list_volc$acpP_PNA_mut_5_vs_ctrl, list_volc$acpP_PNA_mut_6_vs_ctrl, list_volc$acpP_PNA_mut_7_vs_ctrl, list_volc$acpP_PNA_mut_8_vs_ctrl, list_volc$acpP_PNA_mut_9_vs_ctrl)
dev.off()
...
startot <- startot %>% filter(trans_coord %in% c(-20:5)) %>% filter(!grepl("_rev", probe_id)) %>%
...
#startot_upec <- startot_upec  %>% filter(trans_coord %in% c(-20:5)) %>% filter(!grepl("_rev", probe_id))
...
df_bin <- data.frame(above_7 = c(">= 7", "< 7"), 
                     perc_down = c(sum(na.omit(startot_upec$longest_stretch>=7 &
                                         startot_upec$downregulated=="yes" &
                                         startot_upec$logcpm>5))/
                                     sum(na.omit(startot_upec$longest_stretch>=7 &
...
x_ph1 <- sum(na.omit(startot_upec$longest_stretch>=7 & startot_upec$downregulated=="yes" &
                                         startot_upec$logcpm>5))
x_ph2 <- sum(na.omit(startot_upec$longest_stretch<7 & startot_upec$downregulated=="yes" &
                                         startot_upec$logcpm>5))

m_ph <- sum(na.omit(startot_upec$downregulated=="yes" & startot_upec$logcpm>5))
n_ph <- sum(na.omit(startot_upec$downregulated=="no" & startot_upec$logcpm>5))

k_ph1  <- sum(na.omit(startot_upec$longest_stretch>=7 & startot_upec$logcpm>5))
k_ph2  <- sum(na.omit(startot_upec$longest_stretch<7 & startot_upec$logcpm>5))
...
perc <- data.frame(percentage = sapply(0.5:4.5, function(x){
  sum(na.omit(startot_upec_mm$central_mm== x & startot_upec_mm$downregulated=="yes" & startot_upec_mm$logcpm>5) )/ 
    sum(na.omit(startot_upec_mm$central_mm== x& startot_upec_mm$logcpm>5))
}), pos = 0.5:4.5)
...
svg("../analysis/barplots_degenes.svg", height=10, width=10)
